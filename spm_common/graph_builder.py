# spm_common/graph_builder.py
"""
Parses model.json (this project's own tensor-graph input format -- not a
SCALE-Sim concept, and not COSMA-specific either) into two dicts:
  ...
  tensors: tensor id -> Tensor (activations only; weights/bias/network-input
                                 ...
                                 by any SPM-management policy built on this)

A tensor is "tracked" iff it is produced by some layer's `outputs`. Any
input whose `inputs_from` entry is -1 (weights, bias, or the network's own
raw input) is never produced by a layer, so it is naturally excluded -- we
don't need to special-case tensor ids or dtypes to find them.

Lives in spm_common/, not cosma/ or onsram/, because it has zero
algorithm-specific logic: COSMA's ILP and OnSRAM's greedy heuristic both
consume the exact same nodes/tensors dicts this produces (see
onsram/run_onsram.py's own run_onsram(), which imports this module
directly rather than duplicating it -- contrast with e.g.
onsram/onsram_helpers/topology.py, which *is* a deliberate duplicate of
cosma/helpers/topology_builder.py, because that one drives the real
SCALE-Sim simulation and the two papers' simulated numbers must stay
fully isolated from each other; parsing model.json into plain dicts carries
no such risk).
"""
import json
import math
...
if __name__ == '__main__':
    import os
    # model.json lives under cosma/ specifically (COSMA's own real fixture),
    # not next to this file -- this module itself is paper-agnostic, but the
    # only real model.json this repo ships lives in COSMA's directory.
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    here = os.path.join(repo_root, 'cosma')
