# spm_common/spm_allocator.py
"""
A real, live, byte-addressed SPM occupancy tracker that replays any solved
SPM-management plan (a resident_action/spm_plan pair, in the shape both
cosma_Ilp.extract_results() and onsram_helpers.pinning.build_resident_action()
/build_spm_plan() produce) one timestep at a time, verifying every
Create/Preserve/Spill/Retrieve transition is physically consistent (no
double-booking, no freeing a tensor that was never there, no address
collision, never over budget). Takes plain dicts and a byte budget only --
no assumption anywhere in this file about *how* the plan was decided (ILP,
greedy heuristic, or anything else), which is what makes it safe for
cosma/helpers/baseline.py and onsram/onsram_helpers/{scale_sim_runner,
placement,visualize}.py to all import this exact module directly rather
than each keeping their own copy (contrast with the SCALE-Sim-driving
files those modules live in, which *are* deliberately duplicated per-paper
-- see spm_common/__init__.py).

Why this exists: a real SCALE-Sim loop only ever reads resident_action's
per-tensor 'P' flag to decide whether to skip a single tensor's own fetch
-- it has no live, cross-layer bookkeeping of its own, so nothing would
ever catch a genuine capacity violation or address collision if a plan and
the engine's execution ever disagreed. This module is that missing,
independent check -- it doesn't change what gets simulated, it verifies
that what gets simulated is physically realizable at the declared budget.

Standalone: no scalesim/numpy imports, so it can run two ways -- driven
live from inside a real per-layer simulation loop (see cosma/helpers/
baseline.py's _run_layers()/run_cosma_aware(), or onsram_helpers/
scale_sim_runner.py's run_onsram_aware()), or driven standalone against a
solved plan with zero SCALE-Sim involvement (needed for COSMA's
toy_spill_model.json/toy_branching_model.json, which have no real conv
params and can't run through a real simulation pass at all -- see those
files' own _comment fields).
